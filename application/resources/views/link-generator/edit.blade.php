...
<div style="max-width: 800px; margin: 0 auto; padding: 2rem;">
    <div style="margin-bottom: 2rem;">
        ...
        <h1 style="color: #333; margin-bottom: 0.5rem;">ویرایش لینک</h1>
    </div>
    ...
    <form method="POST" action="{{ route('epics.link-generator.update', $link->id) }}" 
        ...
        <div style="margin-bottom: 2rem;">
            ...
            <input type="url" 
                   name="landing_url" 
                   value="{{ old('landing_url', $link->landing_url) }}" 
                   required
        ...
        <div style="margin-bottom: 2rem;">
            ...
            <select name="campaign_id" 
                ...
                @foreach($campaigns as $campaign)
                    <option value="{{ $campaign->id }}" {{ old('campaign_id', $link->campaign_id) == $campaign->id ? 'selected' : '' }}>
                        {{ $campaign->name }}
                    </option>
                @endforeach
            </select>
                ...
                @foreach($utms as $index => $utm)
                    <div class="utm-row" style="display: flex; gap: 0.5rem; margin-bottom: 0.5rem;">
                        ...
                        <button type="button" 
                                ...
                                style="padding: 0.75rem 1rem; background: #dc3545; color: white; border: none; border-radius: 6px; cursor: pointer; {{ count($utms) > 1 ? '' : 'display: none;' }}">
                            حذف
                        </button>
                    </div>
                @endforeach
